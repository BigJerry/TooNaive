...
class DQN(object):
    def __init__(self,gamma,lr,batch_size):
        self.gamma = gamma
        self.lr = lr
        ...
        for piece in batch_transition:
            if piece[3].list_tflag[-1]:
                ...
                with tf.Session() as sess:
                    ...
                    feed_dict = {self.x:x}
                    max_act_Q_value = np.max(self.output.eval(feed_dict=feed_dict))
                y = piece[2] + self.gamma * max_act_Q_value
                ret.append([y,piece[1]])
        return np.array(ret).reshape((len(ret),2))
    
    def _make_input_batch(self,batch_transition):
        ret = []
        for piece in batch_transition:
    ...
    def select_action(self,seq):                                               #ToDo:implement other branch of selection strategy,which is random selection
        if len(seq.list_states) > 3:
            ...
            with tf.Session() as sess:
                sess.run(tf.global_variables_initializer())
                return np.argmax(self.output.eval(feed_dict=feed_dict))   
        else:
    ...
    def optimize(self,batch_transition):                                       
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            x = self._make_input_batch(batch_transition)
...
class Learning(object):
    ...
    def train(self):
        ...
        dqn = DQN(self.gamma,self.lr,self.batch_size)
        env = Environment()
        ...
        for ep in range(self.episode_num):
            ...
            for ts in range(self.timesteps):
                ...
                act = dqn.select_action(self._preprocess_seq(seq))
